...
import csv
import queue
from typing import TypeVar, Callable, Tuple, List, Set, Dict
...
class Graph:
    ...
    def bfs(self, begin_id: str, end_id: str) -> Tuple[List[str], float]:
        """
        Perform a breadth-first search between two vertices.

        :param begin_id: [str] starting vertex id.
        :param end_id: [str] ending vertex id.
        :return: [tuple[list[str], float]] (path, distance) from begin_id to end_id, or ([], 0) if no path.
        """
        ...
        start_v = self.get_vertex_by_id(begin_id)
        ...
        while not q.empty():
            ...
            if cur_id == end_id:
                path, dist = self.build_path(prev, begin_id, end_id)
                self.unvisit_vertices()
                return path, dist

            cur_v = self.get_vertex_by_id(cur_id)

            for nb_id in cur_v.adj:
                nb_v = self.get_vertex_by_id(nb_id)
                if not nb_v.visited:
                    nb_v.visited = True
                    prev[nb_id] = cur_id
                    q.put(nb_id)
    ...
    def a_star(self, begin_id: str, end_id: str,
        ...
        """
        Perform an A* search between two vertices.

        :param begin_id: [str] starting vertex id.
        :param end_id: [str] ending vertex id.
        :param metric: [Callable[[Vertex, Vertex], float]] heuristic distance function.
        :return: [tuple[list[str], float]] (path, distance) from begin_id to end_id, or ([], 0) if no path.

        """
        start_v = self.get_vertex_by_id(begin_id)
        target_v = self.get_vertex_by_id(end_id)
        if start_v is None or target_v is None:
            return [], 0
        ...
        for vertex in self.get_all_vertices():
            ...
            pq.push(float('inf'), vertex)

        dist[begin_id] = 0.0
        first_f = metric(start_v, target_v)
        pq.update(first_f, start_v)

        while not pq.empty():
            cur_f, cur_v = pq.pop()

            if dist[cur_v.id] == float('inf'):
                break

            if cur_v.id == end_id:
                path, total = self.build_path(prev, begin_id, end_id)
                self.unvisit_vertices()
                return path, total

            for nb_id, w in cur_v.adj.items():
                nb_v = self.get_vertex_by_id(nb_id)
                if nb_v.visited:
                    continue

                new_dist = dist[cur_v.id] + w
                if new_dist < dist[nb_id]:
                    dist[nb_id] = new_dist
                    prev[nb_id] = cur_v.id
                    h_val = metric(nb_v, target_v)
                    f_val = new_dist + h_val
                    pq.update(f_val, nb_v)
...
def teleport(system_dict: Dict, start: Tuple[str, str], end: Tuple[str, str]) -> float:
    """
    Shortest path between two vertices across multiple galaxy systems.

    :param system_dict: [dict] mapping galaxy names to system information.
    :param start: [tuple[str, str]] (start_galaxy, start_vertex_id).
    :param end: [tuple[str, str]] (end_galaxy, end_vertex_id).
    :return: [float] shortest distance between the start and end vertices, or float('inf') if no path.

    """
    start_gal, start_id = start
    end_gal, end_id = end

    if start_gal not in system_dict or end_gal not in system_dict:
        return float('inf')

    start_graph: Graph = system_dict[start_gal]["graph"]
    end_graph: Graph = system_dict[end_gal]["graph"]

    if start_id not in start_graph.vertices or end_id not in end_graph.vertices:
        return float('inf')

    if start_gal == end_gal and start_id == end_id:
        return 0.0

    start_node = (start_gal, start_id)
    target_node = (end_gal, end_id)

    target_graph: Graph = system_dict[end_gal]["graph"]
    target_v = target_graph.get_vertex_by_id(end_id)

    def heuristic(node: Tuple[str, str]) -> float:
        gal, v_id = node
        if gal != end_gal:
            return 0.0
        graph = system_dict[gal]["graph"]
        v_obj = graph.get_vertex_by_id(v_id)
        if v_obj is None or target_v is None:
            return 0.0
        return v_obj.euclidean_distance(target_v)

    dist: Dict[Tuple[str, str], float] = {start_node: 0.0}
    seen: Set[Tuple[str, str]] = set()
    heap = []
    counter = itertools.count()

    heapq.heappush(heap, (heuristic(start_node), next(counter), start_node))

    while heap:
        f_val, cnt_val, node = heapq.heappop(heap)
        ...
        gal, v_id = node
        info = system_dict[gal]
        graph: Graph = info["graph"]

        v = graph.get_vertex_by_id(v_id)
        if v is None:
            continue

        for nb_id, w in v.adj.items():
            next_node = (gal, nb_id)
            new_dist = dist[node] + w
            if new_dist < dist.get(next_node, float('inf')):
                dist[next_node] = new_dist
                new_f = new_dist + heuristic(next_node)
                heapq.heappush(heap, (new_f, next(counter), next_node))

        if v_id == info["departure_teleport"]:
            for dest_gal in info["departure_destinations"]:
                if dest_gal not in system_dict:
                    continue
                dest_info = system_dict[dest_gal]
                arrival_id = dest_info["arrival_teleport"]
                next_node = (dest_gal, arrival_id)
                new_dist = dist[node]
                if new_dist < dist.get(next_node, float('inf')):
                    dist[next_node] = new_dist
                    new_f = new_dist + heuristic(next_node)
...
class PriorityQueue:
    ...
    def update(self, new_priority: float, vertex: Vertex) -> None:
        ...
        """
        node = self.locator.pop(vertex.id)
        node[-1] = None
